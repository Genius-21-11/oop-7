        ...
        while (flag) {
            ...
            switch (doing.charAt(0)) {
                case ('+') -> {
                    ...
                    logo.saveLog("LOG: использован метод sum(Rational, Rational) " + calculat.sum(rational1, rational2).toString());
                }
                case ('-') -> {
                    System.out.printf("%s - %s = %s" + "\n", rational1, rational2, calculat.raznoct(rational1, rational2));
                    logo.saveLog("LOG: использован метод raznoct(Rational, Rational) " + calculat.raznoct(rational1, rational2).toString());
                }
                case ('*') -> {
                    System.out.printf("%s * %s = %s" + "\n", rational1, rational2, calculat.multy(rational1, rational2));
                    logo.saveLog("LOG: использован метод multy(Rational, Rational) " + calculat.multy(rational1, rational2).toString());
                }
                case ('/') -> {
                    System.out.printf("%s : %s = %s" + "\n", rational1, rational2, calculat.div(rational1, rational2));
                    logo.saveLog("LOG: использован метод div(Rational, Rational) " + calculat.div(rational1, rational2).toString());
                }
            }
